...
export default function ProductGrid({ products, user, onAddToCart, onToggleFavorite, favorites, onMarkSold }) {
  return (
          ...
          {products.map((p) => {
            ...
            return (
              <motion.div
                ...
                className="group relative overflow-hidden rounded-2xl border border-violet-500/20 bg-gradient-to-b from-[#160a33] to-[#0f0920] p-4 shadow-[0_10px_40px_-10px_rgba(139,92,246,0.4)]"
              >
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        aria-label="Favorite"
                        className={`inline-flex items-center justify-center rounded-md border border-violet-500/20 bg-[#120a28] p-2 text-violet-200 transition hover:bg-white/10 ${isFav ? 'text-pink-400' : ''}`}
                      >
                        <Heart size={16} />
                      </button>
                      <button
                        onClick={() => onAddToCart(p)}
                        className="inline-flex items-center gap-2 rounded-md bg-violet-600 px-3 py-2 text-sm font-medium text-white shadow shadow-violet-600/30 transition hover:bg-violet-500"
                      >
